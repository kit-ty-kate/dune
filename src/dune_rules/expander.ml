...
let expand_version scope pform s =
  ...
  | None ->
    ...
    | None ->
      User_error.raise
        ~loc:(String_with_vars.Var.loc pform)
        [ Pp.textf "Package %S doesn't exist in the current project and isn't installed either." s ]
    | Some lib ->
      value_from_version (Lib_info.version (Lib.info lib))
    end
  | Some p ->
    value_from_version p.version
